...
MidiFilePlayer::MidiFilePlayer(juce::MidiKeyboardState keyState) : Thread("MIDI Player Thread"), keyboardState(keyState),  theMidiFile()
{
...
bool MidiFilePlayer::loadMidiFile(const juce::String path)
{
...
{
    if(canPlay) /*startThread(static_cast<int>(static_cast<Priority>(10)))*/juce::Thread::startThread();
}
...
void MidiFilePlayer::playFile()
{
    ...
    while(i < numTrackEvents && !juce::Thread::threadShouldExit())
    {
        juce::MidiMessage* currMsg = &tracks[1].getEventPointer(i)->message;
        if(!currMsg->isEndOfTrackMetaEvent()) // TODO ERROR CORRECTION - MIGHT NOT BE END OF TRACK MSG
        {
            juce::MidiMessage* nextMsg = &tracks[1].getEventPointer(i + 1)->message;
...
void MidiFilePlayer::printAllTracks()
{
    for(int i = 0; i < numTracks; i++)
    {
        ...
        for(int j = 0; j < tracks[i].getNumEvents(); j++)
            DBG(juce::String(std::to_string(j)) + " @" + std::to_string(tracks[i].getEventPointer(j)->message.getTimeStamp()) + " " + tracks[i].getEventPointer(j)->message.getDescription());
    }
}
